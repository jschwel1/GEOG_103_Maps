...
public class Maps_4 extends JPanel implements ActionListener, KeyListener, MouseListener{
	//======= Instance Fields =============//
	...
	public ArrayList<Place> getListOfPlaces(int quiz){
		...
		if (quiz == 5){
			places.add(new Place("Latin America", new ImageIcon("src/South_America/Blank_South_America_Map.jpg")));
	...
	public void reset(){
		// set up randomized sequence of countries
				...
				int r = (int)(Math.random()*10)-15 + bg.getRed();
				int g = (int)(Math.random()*10)-15 + bg.getGreen();
				int b = (int)(Math.random()*10)-15 + bg.getBlue();
				if (r > 200 || r < 100) 
					r = 150;
				if (g > 200 || g < 100) 
					g = 150;
